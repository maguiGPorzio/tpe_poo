...
public class CustomHBox extends HBox {
    ...
    private int currentLayer=MIN_LAYERS;
    ...
    private ChoiceBox<String> layers = new ChoiceBox<>(layersList);
    ...
    public void setAddLayerAction(EventHandler<ActionEvent> action){
        addLayerButton.setOnAction(event -> {
            String newLayer = "Capa %d".formatted(++currentLayer);
            layersList.add(newLayer);
            layers.setValue(newLayer);
            action.handle(event);
        });
    }
    public void setRemoveLayerAction(EventHandler<ActionEvent> action){
        removeLayerButton.setOnAction(event -> {
            if (currentLayer > MIN_LAYERS) {
                ...
                layersList.remove(layerToRemove);
                layers.getItems().remove(layerToRemove);
                currentLayer--;
                layers.setValue("Capa %d".formatted(currentLayer));
            }
            action.handle(event);
        });
    }
